...
class MRNaiveBayesTrainer(MRJob):
    ...
    def mapper(self, _, line):
        ...
        if docID != "D5":  #skip doc d5 in chinese dataset
            if docClass == "1":
                self.modelStats.setdefault('*TomsPriors', [0, 0])
                self.modelStats['*TomsPriors'][1] +=1
                for word in words:
                    self.modelStats.setdefault('*total_word_count', [0, 0])
                    self.modelStats['*total_word_count'][1] +=1
                    self.modelStats.setdefault(word, [0, 0])
                    self.modelStats[word][1] +=1
                    yield "*!"+word, [0,0]
                    
            else:
                self.modelStats.setdefault('*TomsPriors', [0, 0])
                self.modelStats['*TomsPriors'][0] +=1
                for word in words:
                    self.modelStats.setdefault('*total_word_count', [0, 0])
                    self.modelStats['*total_word_count'][0] +=1
                    self.modelStats.setdefault(word, [0, 0])
                    self.modelStats[word][0] +=1
                    yield "*!"+word, [0,0]
